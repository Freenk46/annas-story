...
export default function CategoriesSection() {
  ...
  // ── Mobile: fade transition when active category changes ───────────────
  useEffect(() => {
  ...
  // ── Mobile: IntersectionObserver per category block ────────────────────
  useEffect(() => {
    const observers = CATEGORIES.map((col, i) => {
      const el = document.getElementById(`category-${col.id}`)
      if (!el) return null
        ...
        <div ref={centerRef} style={{ position: 'relative', zIndex: 1, borderRight: '1px solid var(--border)', overflow: 'hidden' }}>
          <div ref={cardStackRef} style={{ position: 'absolute', top: 0, left: 0, right: 0 }}>
            {CATEGORIES.map((col, i) => (
              <Link
                key={col.id}
                href={`/${locale}/categories/${col.slug}`}
