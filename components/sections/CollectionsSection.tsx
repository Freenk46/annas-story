...
export default function CollectionsSection() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      ...
      setEntered(true)                          // triggers opacity:1 on sticky
      gsap.fromTo(
  ...
  // ── Scroll scrub engine (RAF + lerp, ~1.5 s catchup feel) ────────────────
  useEffect(() => {
    ...
    let rafId: number
    const LERP = 0.04   // 1-(1-0.04)^90 ≈ 97 %  → ~1.5 s at 60fps

    const PARK_TOP     = 72   // px from top of right column
    const PARK_SPACING = 42   // px per stacked item
    ...
    const tick = () => {
      ...
      const raw        = (lerpedY - sectionTop) / vH   // 0 → TOTAL (raw, may exceed)
      const total      = clamp(raw, 0, TOTAL)           // clamped to [0, TOTAL]

      // ── LEFT — numbers ────────────────────────────────────────────────────
      numRefs.current.forEach((el, i) => {
        if (!el) return
        const rel = total - i  // <0 upcoming | 0-1 active range | >1 past
        ...
        if (rel <= 0) {
          ...
          const t = rel / 0.3
          y = lerpFn(-120, 0, t); op = t
        } else if (rel < 0.7) {
          y = 0; op = 1
        } else if (rel < 1) {
          const t = (rel - 0.7) / 0.3
          y = lerpFn(0, 120, t); op = 1 - t
        } else {
          y = 120; op = 0
        }
      ...
      // ── CENTER — translate card stack (natural 1:1 speed, no lerp on stack)
      if (cardStackRef.current) {
        const scrollAmt = clamp(raw, 0, TOTAL - 1) * vH
        cardStackRef.current.style.transform = `translateY(${-scrollAmt}px)`
      }

      // Blur + opacity per card (rAF-driven, lerp already in total)
      cardRefs.current.forEach((el, i) => {
        if (!el) return
        const dist = Math.abs(total - i)
        const blur = clamp(dist * 6, 0, 6)
        const op   = clamp(1 - dist * 0.55, 0.4, 1)
        el.style.filter  = `blur(${blur.toFixed(1)}px)`
        el.style.opacity = op.toFixed(3)
      })

      // Campaign / category side labels — update text on active change
      ...
      // ── RIGHT — nav items (position: absolute, scroll-driven) ────────────
      ...
      const listTop  = rH / 2 - ((TOTAL - 1) / 2) * spacing  // y of first item
      ...
      navRefs.current.forEach((el, i) => {
        ...
        if (rel >= 1) {
          ...
          // CURRENT — traveling from list to park (1:1 with scrollY)
          const parkY = PARK_TOP + parkedSoFar * PARK_SPACING
          const y     = lerpFn(listY, parkY, rel)
          const op    = lerpFn(1,     0.3,   rel)
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          opacity: entered ? 1 : 0,   // entry animation takes over after this
        }}
      >
        {/* ── Decorative S-curve (continues from AboutSection) ─────────── */}
        <svg
          aria-hidden="true"
          style={{
        ...
        {/* ── Campaign label — left of center card ─────────────────────── */}
        <div
          ref={campaignRef}
          style={{
            position: 'absolute',
        ...
        {/* ── Category label — right of center card ────────────────────── */}
        <div
          ref={categoryRef}
          style={{
            position: 'absolute',
        ...
        >
          {/* Inner stack — translated by RAF at natural 1:1 scroll speed */}
          <div
            ref={cardStackRef}
            ...
            {COLLECTIONS.map((col, i) => (
              <Link
                ...
                style={{
                  ...
                  padding: '0 32px',
                  textDecoration: 'none',
                  // blur/opacity driven by RAF
                }}
              >
                {/* Visual — video if available, placeholder otherwise */}
                <div style={{
        ...
        >
          ...
          {/* All 11 nav items — position:absolute, entirely RAF-driven */}
          {COLLECTIONS.map((col, i) => (
            <div
              key={col.id}
